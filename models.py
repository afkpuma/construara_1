...
class Cliente(db.Model):
        __tablename__ = 'clientes' # Boa prática: define o nome da tabela explicitamente
...
class Andaime(db.Model):
        __tablename__ = 'andaimes' # Boa prática: define o nome da tabela explicitamente
        id = db.Column(db.Integer, primary_key=True)
...
class Locacao(db.Model):
        __tablename__ = 'locacoes' # Boa prática: define o nome da tabela explicitamente
        id = db.Column(db.Integer, primary_key=True)
        ...
        def to_dict(self):
            return {
                ...
                'andaimes': [la.andaime.to_dict() for la in self.andaimes_locados]
            }

class LocacaoAndaime(db.Model):
        __tablename__ = 'locacao_andaimes' # Boa prática: define o nome da tabela explicitamente
        id = db.Column(db.Integer, primary_key=True)
